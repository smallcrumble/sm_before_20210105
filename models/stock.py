...
class StockMove(models.Model):
	...
	def _action_confirm(self, merge=True, merge_into=False):
		#inherit dari stock_move di mrp_subcontracting
				...
				if float_is_zero(move.product_uom_qty, precision_rounding=move.product_uom.rounding):
					...
					for (location_id, lot_id, package_id, owner_id), quantity in available_move_lines.items():
						...
						if float_is_zero(available_quantity, precision_rounding=rounding):
							continue
						taken_quantity, taken_quantity1, taken_quantity2 = move._update_reserved_quantity(need, need1, need2, min(quantity, available_quantity), min(quantity1, available_quantity1), min(quantity2, available_quantity2), location_id, lot_id, package_id, owner_id)
						if float_is_zero(taken_quantity, precision_rounding=rounding):
							continue
